...
class CustomerEngagement(db.Model):
    ...
    # Relationships
    client = db.relationship('Client', backref='customer_engagements')
    post = db.relationship('Post', backref='customer_engagements')
    responder = db.relationship('User', backref='engagement_responses')
...
class UnifiedInboxMessage(db.Model):
    ...
    suggested_template = db.relationship('CommentTemplate')
    thread_messages = db.relationship('MessageThread', backref='original_message', lazy='dynamic')
...
class EngagementAutomation(db.Model):
    ...
    # Relationships
    client = db.relationship('Client', backref='engagement_automations')
