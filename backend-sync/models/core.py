"""
Core Database Models for Kuwait Social AI
"""
...
# Import db from the package init
from . import db
...
class User(db.Model):
    ...
    # Additional relationships to fix missing back_populates
    client_errors = db.relationship('ClientError', back_populates='user')
    engagement_responses = db.relationship('CustomerEngagement', back_populates='responder')
...
class Client(db.Model):
    ...
    ai_credits_limit = db.Column(db.Integer, default=1000)
    ...
    # Additional relationships to fix missing back_populates (only for existing models)
    campaigns = db.relationship('Campaign', back_populates='client')
    competitors = db.relationship('Competitor', back_populates='client')
    hashtag_strategies = db.relationship('HashtagStrategy', back_populates='client')
    trending_hashtags = db.relationship('TrendingHashtag', back_populates='client')
    hashtag_combinations = db.relationship('HashtagCombination', back_populates='client')
    api_keys = db.relationship('APIKey', back_populates='client')
    
    # From engagement_models.py
    customer_engagements = db.relationship('CustomerEngagement', back_populates='client')
    comment_templates = db.relationship('CommentTemplate', back_populates='client')
    inbox_messages = db.relationship('UnifiedInboxMessage', back_populates='client')
    response_metrics = db.relationship('ResponseMetrics', back_populates='client')
    customer_profiles = db.relationship('CustomerProfile', back_populates='client')
    engagement_automations = db.relationship('EngagementAutomation', back_populates='client')
    
    # From reporting_models.py
    report_templates = db.relationship('ReportTemplate', back_populates='client')
    generated_reports = db.relationship('GeneratedReport', back_populates='client')
    roi_tracking = db.relationship('ROITracking', back_populates='client')
    analytics_dashboards = db.relationship('AnalyticsDashboard', back_populates='client')
    metric_alerts = db.relationship('MetricAlert', back_populates='client')
    
    # From api_key.py
    api_keys = db.relationship('APIKey', back_populates='client')
    
    # From hashtag_models.py
    hashtag_groups = db.relationship('HashtagGroup', back_populates='client')
    hashtag_performance = db.relationship('HashtagPerformance', back_populates='client')
    hashtag_recommendations = db.relationship('HashtagRecommendation', back_populates='client')
    
    # From missing_models.py
    scheduled_posts = db.relationship('ScheduledPost', back_populates='client')
    
    # From kuwait_features_models.py
    kuwait_features = db.relationship('KuwaitFeature', back_populates='client')
    ...
    def to_dict(self):
        return {
            ...
            'ai_credits_remaining': self.ai_credits_limit - self.ai_credits_used
        }

class Feature(db.Model):
    """Platform features that can be enabled for clients"""
    __tablename__ = 'features'
    
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), unique=True, nullable=False)
    description = db.Column(db.Text)
    category = db.Column(db.String(50))  # content, analytics, engagement, kuwait_specific
...
class Post(db.Model):
    ...
    content_ar = db.Column(db.Text)  # Arabic version
    ...
    # Additional relationship to fix missing back_populates
    customer_engagements = db.relationship('CustomerEngagement', back_populates='post')
...
class CompetitorAnalysisOld(db.Model):
    """Competitor tracking"""
