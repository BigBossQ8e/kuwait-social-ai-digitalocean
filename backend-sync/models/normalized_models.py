...
class CompetitorAnalysis(db.Model):
    ...
    competitor = db.relationship('Competitor', back_populates='analyses')
...
class HashtagStrategy(db.Model):
    ...
    # Relationships (replacing JSON columns)
    trending_hashtags = db.relationship('TrendingHashtag', back_populates='strategy', lazy='dynamic')
    recommended_combinations = db.relationship('HashtagCombination', back_populates='strategy', lazy='dynamic')
...
class TrendingHashtag(db.Model):
    ...
    strategy_id = db.Column(db.Integer, db.ForeignKey("hashtag_strategies.id"), nullable=True)
    strategy = db.relationship("HashtagStrategy", back_populates="trending_hashtags")
    hashtag = db.Column(db.String(100), nullable=False, index=True)
...
class HashtagCombination(db.Model):
    ...
    client_id = db.Column(db.Integer, db.ForeignKey('clients.id'), nullable=False, index=True)
    strategy_id = db.Column(db.Integer, db.ForeignKey("hashtag_strategies.id"), nullable=True)
    strategy = db.relationship("HashtagStrategy", back_populates="recommended_combinations")
    ...
    # Relationships
    client = db.relationship('Client')
    hashtags = db.relationship('HashtagCombinationItem', back_populates='combination', lazy='dynamic', 
...
class HashtagCombinationItem(db.Model):
    ...
    # Relationships
    combination = db.relationship('HashtagCombination', back_populates='hashtags')
